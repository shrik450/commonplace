...
// - `searchItems` builds its snippet from the `transcript` column, marking the
//   hit with `\u0002` and `\u0003`. The snippet is plain text; L2 emits no
//   HTML.
// - `indexItem` deletes then inserts, so indexing twice leaves one row.
...
import type { FtsRow } from "../../src/store/fts";
import { indexItem, removeItem, searchItems } from "../../src/store/fts";
import {
  ITEM_FILES,
  deleteItemDir,
  itemDir,
...
const ITEM_D = "aaaaaaaa-0000-4000-8000-000000000004";
...
function makeItem(id: string, userId: string, title: string): Item {
  return {
    id,
    user_id: userId,
    kind: "article",
...
function makeRequest(
  overrides: Partial<FetchRequest> & { id: string; user_id: string },
): FetchRequest {
  return {
    item_id: null,
...
function ftsRow(
  overrides: Partial<FtsRow> & { item_id: string; user_id: string },
): FtsRow {
  return {
    title: "A field guide",
    author: null,
    transcript:
      "The quick brown fox jumps over the lazy dog. A wombat digs a burrow.",
    ...overrides,
  };
}
...
describe("src/store/fts", () => {
  test("indexes an item and finds it again", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));

    const hits = searchItems(db, ALICE, "wombat", 10);
    expect(hits).toHaveLength(1);
    expect(hits[0]!.item_id).toBe(ITEM_A);
    expect(hits[0]!.title).toBe("A field guide");
  ...
  test("returns page text unchanged, adding no markup of its own", () => {
    // The transcript comes from a captured page. L2 hands it back byte for
    ...
    indexItem(
      ...
      ftsRow({
        item_id: ITEM_A,
        user_id: ALICE,
        transcript: 'A <script>alert("x")</script> wombat and friends.',
      }),
    );
  ...
  test("returns an empty array when nothing matches", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    expect(searchItems(db, ALICE, "aardvark", 10)).toEqual([]);
    db.close();
  });

  test("re-indexing the same item leaves one row", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    indexItem(
      db,
      ftsRow({ item_id: ITEM_A, user_id: ALICE, title: "A second guide" }),
    );

    const rows = db
      .query<{ n: number }, [string]>(
        "SELECT COUNT(*) AS n FROM items_fts WHERE item_id = ?",
      )
      .get(ITEM_A)!;
    ...
    const hits = searchItems(db, ALICE, "wombat", 10);
    expect(hits).toHaveLength(1);
    expect(hits[0]!.title).toBe("A second guide");
    db.close();
  });
  ...
  test("removeItem drops the item from the index", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    expect(searchItems(db, ALICE, "wombat", 10)).toHaveLength(1);

    removeItem(db, ALICE, ITEM_A);
    expect(searchItems(db, ALICE, "wombat", 10)).toEqual([]);
    expect(
      db.query<{ n: number }, []>("SELECT COUNT(*) AS n FROM items_fts").get()!.n,
    ).toBe(0);
    db.close();
  });
  ...
  test("removeItem cannot drop another user's item", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_B, user_id: BOB }));

    removeItem(db, ALICE, ITEM_B);

    expect(searchItems(db, BOB, "wombat", 10)).toHaveLength(1);
    db.close();
  });
  ...
  test("one user's search never returns another user's item", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    indexItem(db, ftsRow({ item_id: ITEM_B, user_id: BOB }));

    const forAlice = searchItems(db, ALICE, "wombat", 10);
    expect(forAlice).toHaveLength(1);
    expect(forAlice[0]!.item_id).toBe(ITEM_A);

    const forBob = searchItems(db, BOB, "wombat", 10);
    expect(forBob).toHaveLength(1);
  ...
  test("honours the limit and orders by rank", () => {
    const db = seededFtsDb();
    insertItem(db, makeItem(ITEM_C, ALICE, "A third guide"));
    insertItem(db, makeItem(ITEM_D, ALICE, "A fourth guide"));
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    indexItem(db, ftsRow({ item_id: ITEM_C, user_id: ALICE }));
    indexItem(
      db,
      ftsRow({
        item_id: ITEM_D,
        user_id: ALICE,
        transcript: "A wombat, a wombat, and one more wombat.",
      }),
    );

    expect(searchItems(db, ALICE, "wombat", 10)).toHaveLength(3);

    const limited = searchItems(db, ALICE, "wombat", 2);
    expect(limited).toHaveLength(2);
    expect(limited[0]!.rank).toBeLessThanOrEqual(limited[1]!.rank);
    db.close();
  });

  test("matches a phrase query", () => {
    const db = seededFtsDb();
    indexItem(db, ftsRow({ item_id: ITEM_A, user_id: ALICE }));
    expect(searchItems(db, ALICE, '"quick brown fox"', 10)).toHaveLength(1);
    expect(searchItems(db, ALICE, '"brown quick fox"', 10)).toEqual([]);
    db.close();
  });
});

describe("src/store/files layout", () => {
  test("ITEM_FILES names the four files of an item", () => {
...
describe("src/store/files path checking", () => {
  ...
  test("itemDir rejects a bad user id and a bad item id", () => {
    for (const bad of badIds) {
      expect(caughtAppErrorCode(() => itemDir("/items", bad, ITEM_A))).toBe(
        "STORE_INVALID_PATH",
      );
      expect(caughtAppErrorCode(() => itemDir("/items", ALICE, bad))).toBe(
        "STORE_INVALID_PATH",
      );
    }
  });
  ...
  test("itemDir rejects an uppercase UUID", () => {
    ...
    // filesystem, and newId() mints lowercase.
    expect(
      caughtAppErrorCode(() => itemDir("/items", ALICE, ITEM_A.toUpperCase())),
    ).toBe("STORE_INVALID_PATH");
    expect(
      caughtAppErrorCode(() => itemDir("/items", ITEM_A.toUpperCase(), ITEM_A)),
    ).toBe("STORE_INVALID_PATH");
    expect(
      caughtAppErrorCode(() =>
        itemDir("/items", ALICE, "AAAAAAAA-0000-4000-8000-000000000001"),
      ),
    ).toBe("STORE_INVALID_PATH");
  });
  ...
  test("writeItemFile rejects a traversal id and writes nothing", async () => {
    ...
    expect(
      await rejectedAppErrorCode(() =>
        writeItemFile(root, "../../etc", ITEM_A, "transcript.txt", "x"),
      ),
    ).toBe("STORE_INVALID_PATH");
    expect(
      await rejectedAppErrorCode(() =>
        writeItemFile(root, ALICE, "../../etc/passwd", "transcript.txt", "x"),
      ),
    ).toBe("STORE_INVALID_PATH");
  ...
  test("readItemFile, itemFilesPresent, and deleteItemDir reject a traversal id", async () => {
    const root = await tempRoot();
    expect(
      await rejectedAppErrorCode(() =>
        readItemFile(root, ALICE, "../../etc/passwd", "transcript.txt"),
      ),
    ).toBe("STORE_INVALID_PATH");
    expect(
      await rejectedAppErrorCode(() => itemFilesPresent(root, "..", ITEM_A)),
    ).toBe("STORE_INVALID_PATH");
    expect(
      await rejectedAppErrorCode(() => deleteItemDir(root, "..", ITEM_A)),
    ).toBe("STORE_INVALID_PATH");
  });
});

// Sealed acceptance test for milestone 0. Do not edit this file.
//
...
describe("scripts/verify buildSummary", () => {
  ...
  test("reports not ok when a seal does not match", () => {
    expect(buildSummary({ ...clean, seals: "mismatch" }).ok).toBe(false);
  });
...
describe("scripts/seal", () => {
  test("computeSeals hashes this sealed file", async () => {
    const seals = await computeSeals(join(repoRoot, "test", "acceptance"));
    const key = "test/acceptance/00-foundation.test.ts";

    expect(Object.keys(seals)).toContain(key);

    const hasher = new Bun.CryptoHasher("sha256");
    hasher.update(await Bun.file(import.meta.path).arrayBuffer());
    expect(seals[key]).toBe(hasher.digest("hex"));
    expect(seals[key]).toMatch(/^[0-9a-f]{64}$/);
  });

  test("checkSeals reports ok when the maps match", () => {
    const recorded = { "test/acceptance/00-foundation.test.ts": "aa", "b": "bb" };
    expect(checkSeals(recorded, { ...recorded })).toEqual({
      ok: true,
      changed: [],
      missing: [],
      extra: [],
  ...
  test("checkSeals reports a changed file", () => {
    const result = checkSeals({ a: "aa", b: "bb" }, { a: "aa", b: "cc" });
  ...
  test("checkSeals reports a missing file", () => {
    const result = checkSeals({ a: "aa", b: "bb" }, { a: "aa" });
    expect(result.ok).toBe(false);
  ...
  test("checkSeals reports an extra file", () => {
    const result = checkSeals({ a: "aa" }, { a: "aa", b: "bb" });
    expect(result.ok).toBe(false);
